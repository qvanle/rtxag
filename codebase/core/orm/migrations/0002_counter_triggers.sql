...
CREATE OR REPLACE FUNCTION fn_sync_mcp_record_count() RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE mcp_collections
...
DROP TRIGGER IF EXISTS trg_mcp_record_count ON mcp_records;
CREATE TRIGGER trg_mcp_record_count
AFTER INSERT OR DELETE ON mcp_records
FOR EACH ROW EXECUTE FUNCTION fn_sync_mcp_record_count();
